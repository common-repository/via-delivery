...
<form id="via-order-form" method="post" data-action="/wp-json/woo-viadelivery/order/<?= $order->getId() ?>">
    <div class="order-content" style="display: flex; flex-direction: column;">
        ...
        <div>
            ...
            <nav class="nav-tab-wrapper woo-nav-tab-wrapper" data-tabs-content-level="1">
                <?php foreach ($tabs as $id => $tabname) { ?>
                    ...
                    class="nav-tab dpd-tab <?= $id == $activeTab ? 'nav-tab-active' : '' ?>"
        ...
        <div class="tab-wrapper" style="flex-grow: 1; height: 100%">
            <?php foreach ($tabs as $id => $tabname) { ?>
                <div class="via-tab-content via-tab-content-<?= $id ?>"
                     id="<?php echo $id; ?>"
                     <?= $id == $activeTab ? '' : 'style="display:none;"' ?>
                     style="height: 100%"
                >
